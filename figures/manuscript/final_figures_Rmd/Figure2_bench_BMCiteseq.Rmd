...
legend.celltypes <- ggpubr::get_legend(
  fig1.A +
    scale_size(limits = c(0,max(metrics$size[metrics$gamma == 75])),
               breaks = c(100,200,300,400,2000,4000))  +guides(
                 colour = guide_legend(ncol =2,title = "Cell types",
...
figS2A <-  cowplot::plot_grid(umap.sc.multi + 
                                NoLegend() +
                                scale_size(limits = c(0,max(metrics$size[metrics$gamma == 75])),
                                           breaks = c(50,100,200,400))+ ggtitle("SuperCell Protein+RNA"),
                              umap.sc.adt  + 
                                NoLegend() +
                                scale_size(limits = c(0,max(metrics$size[metrics$gamma == 75])),
                                           breaks = c(50,100,200,400)) +
                              ...
                              umap.sc.rna + 
                                NoLegend() +
                                scale_size(limits = c(0,max(metrics$size[metrics$gamma == 75])),
                                           breaks = c(50,100,200,400)) + ggtitle("SuperCell RNA"),
                              umap.sea.mofa + 
                                NoLegend() +
                                scale_size(limits = c(0,max(metrics$size[metrics$gamma == 75])),
                                           breaks = c(50,100,200,400)) + ggtitle("SEACells Protein+RNA"),
                              umap.sea.adt + 
                                NoLegend() +
                                scale_size(limits = c(0,max(metrics$size[metrics$gamma == 75])),
                                           breaks = c(50,100,200,400)) + ggtitle("SEACells Protein"),
                              umap.sea.rna + 
                                NoLegend() +
                                scale_size(limits = c(0,max(metrics$size[metrics$gamma == 75])),
                                           breaks = c(50,100,200,400)) + ggtitle("SEACells RNA"),
                              umap.metaq.multi + 
                                NoLegend() +
                                scale_size(limits = c(0,max(metrics$size[metrics$gamma == 75])),
                                           breaks = c(50,100,200,400)) + ggtitle("MetaQ Protein+RNA"),
                              umap.metaq.adt + 
                                NoLegend() +
                                scale_size(limits = c(0,max(metrics$size[metrics$gamma == 75])),
                                           breaks = c(50,100,200,400)) + ggtitle("MetaQ Protein"),
                              umap.metaq.rna + 
                                NoLegend() +
                                scale_size(limits = c(0,max(metrics$size[metrics$gamma == 75])),
                                           breaks = c(50,100,200,400)) + ggtitle("MetaQ RNA"),
                              umap.sc,legend.celltypes,
                              umap.mc.rna + 
                                NoLegend() +
                                scale_size(limits = c(0,max(metrics$size[metrics$gamma == 75])),
                                           breaks = c(50,100,200,400)) + ggtitle("MetaCell2 RNA"),
                              ncol = 3
)
...
```{r}
pdf(file = "../../../pdf_files/FigS2/FigS2Anew_WilcoxHeatmap.pdf",height = 12,width=5)
heatmap_wilcox
dev.off()
```
...
```{r}
bm.pseudobulk <- AggregateExpression(bm,group.by = "celltype.l2",slot = "counts",return.seurat = T)
bm.pseudobulk <- NormalizeData(bm.pseudobulk)

distances <- dist(as.matrix(Matrix::t(GetAssayData(bm.pseudobulk,assay = "RNA"))))
...
```{r}
#pct.to.shuffle <- c(0,25,50,75,100)
pct.to.shuffle <- c(0,5,10,20,50)
guiding_label_cols <- paste0("guiding_label_",pct.to.shuffle,"_pct_shuffled")
names(pct.to.shuffle) <- guiding_label_cols
...
for (p in guiding_label_cols) {
  ...
  res[[p]] <- seurat.mc.multi

  res[[p]]@meta.data$pct_shuffle <- pct.to.shuffle[p]
  
  res_df <- rbind(res_df,res[[p]]@meta.data )
}


unsup  <- SCimplify_for_Seurat_v5(
    seurat,
    assay = c(opt$Mod1assay),
    reduction = list(opt$Mod1red),
    dims = list(opt$Mod1comp),
...
unsup$label_col <- unsup$celltype.l2
unsup$label_col_with_unknown <- NA
unsup$label_col_with_unknown_purity <- NA
unsup$pct_shuffle <- "unsupervised"

boxplot(bm.supercell.rna$celltype.l2_purity,
        unsup$celltype.l2_purity)

res_df <- rbind(res_df,unsup@meta.data )

ggplot(res_df,aes(x= as.character(pct_shuffle),y = celltype.l2_purity)) + geom_boxplot()
ggplot(res_df,aes(x= as.character(pct_shuffle),y = size)) + geom_boxplot()


table(seurat$label_col,seurat$celltype.l2)
```
...
for (p in c(100,75,50,25,0)) {
  ...
  plot.list[[paste0("pct_",p)]] <- DimPlotSC(seurat = bm,
    ...
    scale_size(limits = c(0,max(bench.75.no.random.semi.sup$size)),breasks = c(50,100,150,200))  +
    guides(fill=guide_legend(ncol =2,title = "Cell types",
                             override.aes = list(size=4)),
           colour = "none",
           size = guide_legend(title = "Size"))
