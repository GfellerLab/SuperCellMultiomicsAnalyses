...
bm.seacells.adt <- readRDS("../../output/bmCiteSeq_backup/seacellsADT//g75/seurat.cite.mc.rds")
bm.seacells.adt <- addCellType(bm.seacells.adt)

bm.seacells.rna <- readRDS("../../output/bmCiteSeq_backup/seacellsRNA//g75/seurat.cite.mc.rds")
...
```
```{r}
pdf(file = "figS2A.pdf",height = 11,width=11)
figS2A
dev.off()
```
...
input.colors.cite <- c(colorblind_pal()(8)[2:4],"darkgrey")
...
```{r}
pdf("fig2DEF.pdf",height = 5.25,width = 13.75)
Fig2DEF
dev.off()
```
...
figS2BCD <- cowplot::plot_grid(cowplot::plot_grid(compactness_pca.bmc.cite.g75+ NoLegend() + 
                                                    theme(axis.title.x = element_blank()),
...
```
```{r}
pdf(file = "figS2BCD.pdf",height = 12,width=5)
figS2BCD
dev.off()
```
```{r}
...
figS2B <- cowplot::plot_grid(purity.bmc.cite.g20+ NoLegend() + theme(axis.title.x = element_blank()),
...
```{r fig.height=6.5,fig.width=14}
pdf(file = "figS2B.pdf",height = 6.5,width=14)
figS2B
dev.off()
```
...
figS2C <- cowplot::plot_grid(figSupCov+theme(plot.margin = unit(c(50,25,50,25),units = "pt")),
                             wrap_plots(nFeature_RNA, nCount_RNA,ncol = 1),rel_widths = c(3,1))

figS2C
```
```{r}
pdf(file = "figS2C.pdf",height = 5,width=9)
figS2C
dev.off()
```
...
```{r}
pdf("fig2GH.pdf",height = 5,width = 17.5)
fig2GH
dev.off()
```
...
```{r}
pdf("figS2E.pdf",height = 6,width = 5)
sig.celltype.l2_purity.g75.major.type + NoLegend()
dev.off()
```
...
```{r}
pdf("figS2F.pdf",height = 7,width = 3)
...
multimodal_compactness.75.semi.sup <- ggplot(bench.75.no.random.semi.sup[bench.75.no.random.semi.sup$input == "Protein+RNA",],aes(y=multimodal_compactness,x = pct_known,fill = input)) + 
  geom_boxplot(outlier.shape = NA)  + theme_bw() + 
  theme(legend.position = "bottom")  + scale_fill_manual(values = input.colors.cite) + ylab("Multimodal Compactness")  +  # Hides outliers
  coord_cartesian(ylim = c(0,0.065))   + NoLegend() + 
  geom_signif(comparisons = comparisons.semi.sup,
              y_position = 0.02,
              step_increase=0.015,
              map_signif_level=T,
...
pdf("fig2J_semi_sup.pdf",height = 2.75,width = 7)
wrap_plots(purity.pbmc.multiome.g75.semi.sup,multimodal_compactness.75.semi.sup,multimodal_separation.75.semi.sup,nrow = 1)
dev.off()
```
...
pdf("figS3_semi_sup_bm_cite.pdf", height = 4,width = 18)
wrap_plots(plot.list[[2]],plot.list[[3]],plot.list[[4]],plot.list[[5]],guide_area(),guides = "collect",nrow = 1) & 
  theme(legend.spacing.y = unit(-0.35, "line"),legend.key.size = unit(1, "line"))
...
multimodal_compactness.75.semi.sup.rna <- ggplot(bench.75.no.random.semi.sup[bench.75.no.random.semi.sup$input == "RNA",],aes(y=multimodal_compactness,x = pct_known,fill = input)) + 
  geom_boxplot(outlier.shape = NA)  + theme_bw() + 
  theme(legend.position = "bottom")  + scale_fill_manual(values = input.colors.cite) + ylab("Multimodal Compactness")  +  # Hides outliers
  coord_cartesian(ylim = c(0,0.11))   + NoLegend() + 
  geom_signif(comparisons = comparisons.semi.sup,
              y_position = 0.06,
              step_increase=0.025,
              map_signif_level=T,
...
pdf("fig2J_semi_sup_rna_metrics.pdf",height = 2.75,width = 7)
wrap_plots(purity.pbmc.multiome.g75.semi.sup.rna,multimodal_compactness.75.semi.sup.rna,multimodal_separation.75.semi.sup.rna,nrow = 1)
dev.off()
```
...
pdf("figS3_semi_sup_bm_cite_rna_umaps.pdf", height = 4,width = 18)
wrap_plots(plot.list[[2]],plot.list[[3]],plot.list[[4]],plot.list[[5]],guide_area(),guides = "collect",nrow = 1) & 
  theme(legend.spacing.y = unit(-0.35, "line"),legend.key.size = unit(1, "line"))
...
pdf("figS3_semi_sup_bm_rna_acrprot.pdf",height = 3,width = 3.5)
acr.prot.supercell.rna.semi.sup + NoLegend()
dev.off()
```
