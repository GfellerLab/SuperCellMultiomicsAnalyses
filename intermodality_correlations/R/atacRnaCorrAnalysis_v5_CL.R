...
if (opt$compactnessSeparation) {
  ...
  membership_df <- data.frame("membership" = paste0("Metacell_",na.exclude(seurat.mc@misc$membership)),
                              row.names = colnames(seurat.sc)[!is.na(seurat.mc@misc$membership)])
  
  seurat.mc$compactness_pca <- mc_compactness(cell.membership = membership_df, 
                                                    sc.obj = seurat.sc,
                                                    sc.reduction = "pca_diffusion")
  seurat.mc$compactness_lsi <- mc_compactness(cell.membership = membership_df, 
                                                     sc.obj = seurat.sc,
                                                     sc.reduction = "lsi_diffusion")
  seurat.mc$separation_pca <- mc_separation(cell.membership = membership_df, 
                                                  sc.obj = seurat.sc,
                                                  sc.reduction = "pca_diffusion")
  seurat.mc$separation_lsi <- mc_separation(cell.membership = membership_df, 
                                                   sc.obj = seurat.sc,
                                                   sc.reduction = "lsi_diffusion")
  ...
  # source("R/functions/innerNormalizedVariance.R")
  source("R/functions/metacellCompactnessSeparation.R")
  seurat.mc$asw_lsi_diffusion <- mc_ASW(seurat.mc = seurat.mc,
                                               seurat.sc = seurat.sc,
  ...
  # print(is(seurat.mc))
  # if(opt$gamma > 49) {
  #   seurat.mc$innerNormVar <- mc_INV(cell.membership = membership_df, 
  #                                          sc.obj = seurat)
  # }
  
  seurat.mc$asw_pca_diffusion <- mc_ASW(seurat.mc  = seurat.mc,
                                              seurat.sc = seurat.sc,
                                              cell.membership = membership_df, 
