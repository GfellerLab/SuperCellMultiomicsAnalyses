...
p3 <- DimPlot(hspc, reduction = "wnn.umap", label = TRUE, label.size = 2.5, repel = TRUE) + ggplot2::ggtitle("WNN")
...
genes <- rownames(hspc[["RNA"]]@counts)[Matrix::rowSums(hspc[["RNA"]]@counts)> ncol(hspc)*0.005]
