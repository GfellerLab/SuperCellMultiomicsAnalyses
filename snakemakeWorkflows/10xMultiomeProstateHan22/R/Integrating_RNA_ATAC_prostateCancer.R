...
load("rna.All.combined.integrated.filter.Rdata")
load("atac.prostate.merge.Rdata")
atac.prostate.merge <- subset(atac.prostate.merge,cells = colnames(rna.All.combined.integrated.filter))

#################add ATAC assay to RNA assay
rna.All.combined.integrated.filter[["ATAC"]]<- atac.prostate.merge
DefaultAssay(rna.All.combined.integrated.filter) <- "ATAC"
rna.All.combined.integrated.filter <- RunTFIDF(rna.All.combined.integrated.filter)
rna.All.combined.integrated.filter <- FindTopFeatures(rna.All.combined.integrated.filter, min.cutoff = 'q0')
rna.All.combined.integrated.filter <- RunSVD(rna.All.combined.integrated.filter)

DefaultAssay(rna.All.combined.integrated.filter) <- "ATAC"
rna.All.combined.integrated.filter <- harmony::RunHarmony(
...
pdf(file="umap.all.harmony.dims.pdf",width=1200,height=480)
...
# pdf(file="umap.final.pdf",width=1800,height=600)
# p1 + p2 + p3
# dev.off()
# #######Geneactivity
# DefaultAssay(rna.All.combined.integrated.filter) <- 'Geneactivity'
# pdf(file="fp.Gene_activities.pdf",width=1200,height=600)
# FeaturePlot(
...
# 
# pdf(file="Corplot.Krt8.pdf",width=600,height=480)
...
# dev.off()
# pdf(file="Corplot.Ptprc.pdf",width=600,height=480)
...
# 
# pdf(file="Cellprop.pdf",width=600,height=480)
...
# dev.off()
# 
# pdf(file="Cellprop_no_WT.pdf",width=600,height=480)
...
# pdf(file="Umap_prostate_filter_0.08_1.pdf",width=600,height=480)
...
# 
# pdf(file="umap.prostate.filter.split.pdf",width=1200,height=300)
...
# dev.off()## 
# 
# pdf(file="fp.Lineage.filter.pdf",width=1200,height=1000)
...
#                 '#712820', '#DCC1DD', '#CCE0F5', '#CCC9E6', '#625D9E', '#68A180', '#3A6963',
#                 '#968175')
# pdf(file="1.pdf",width=600,height=500)
