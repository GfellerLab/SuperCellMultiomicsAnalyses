...
for (sampleName in sampleNames) {
  ...
  if (opt$RNAmetacells == "LogNormalize") {
    ...
    pbmcMC <- SCimplify_for_Seurat(seurat = pbmc.smp,
                                   ...
                                   dims = list(opt$ADTcomp, opt$RNAcomp),
                                   graph.name = "knn",
                                   label = "celltype.l1.5",
                                   kernel = T,
                                   gamma = opt$gamma)
...
pbmc.combined <- RunUMAP(pbmc.combined, dims = c(opt$ADTcomp), reduction = "pca", reduction.name = "rna.umap")
...
pbmc.combined <- FindMultiModalNeighbors(
  ...
  dims.list = list(opt$ADTcomp, opt$RNAcomp),
