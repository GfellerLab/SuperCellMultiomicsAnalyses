...
pbmc[["SCT"]] <- as(object = pbmc[["SCT"]], Class = "Assay")

DefaultAssay(pbmc) <- "ATAC"
pbmc <- FindTopFeatures(pbmc, assay="ATAC", min.cutoff = 2000)

mofa <- create_mofa(pbmc, assays = c("SCT","ATAC"))
mofa
...
pbmc[["mofa"]] <- CreateDimReducObject(embeddings = Z,key = "MOFA")

adata <- anndata::AnnData(X = Matrix::t(GetAssayData(object = pbmc,
                                                     layer = "counts",assay = "RNA")),
			                             obs = pbmc@meta.data,
						     #raw = adata.raw,
						     obsm = list("X_mofa" = pbmc[["mofa"]]@cell.embeddings))
