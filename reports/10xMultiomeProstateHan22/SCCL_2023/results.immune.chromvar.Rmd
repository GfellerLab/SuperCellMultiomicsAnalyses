...
```{r}
#combined.metacells <- readRDS("immune.combined.metacells.local.frags_chromvar_mouse.rds")
combined.metacells <- readRDS("immune.combined.metacells.annotated.local.frags.rds")

chromvar.res <- readRDS("../../../output/10xMultiomeProstateHan22/logNorm/macs2_major_type/immune_hoccomoco_motifs/combined.metacells.with.macs.peak.rds")

combined.metacells[['chromvar']] <- chromvar.res[["chromvar_pfm"]]
combined.metacells[['peaks']] <- chromvar.res[["peaks"]]
...
plot <- DimPlot.SuperCell(combined.metacells,reduction = "wnn.umap",group.by = "fine_celltype_2",cols = my36colors)
LabelClusters(plot, id = "fine_celltype_2", size = 5, repel = T,  box.padding = 1)
```
```{r}
DimPlot.SuperCell(combined.metacells,reduction = "wnn.umap",group.by = "wsnn_res.0.5",cols = my36colors)
```
```{r}
combined.macro.dc.metacells <- combined.metacells[,combined.metacells$fine_celltype_2 %in% c("Macro_Fn1_high",                                                                    "Macro_Cd86_high",
"Macro_Trem2_high","DC")]
```
...
```{r}
DimPlot.SuperCell(combined.macro.dc.metacells,reduction = "wnn.umap",label.box = T,label = T,repel = T,cols = mycolors) + xlim(c(0,10)) + ylim(c(-11,-1)) 

```

```{r}
VlnPlot(combined.macro.dc.metacells,c("Clec10a","Tnfsf13b","Thbs1","Mrc1","Trem2","Spp1","Cxcl10"),stack = T,fill.by = "ident",cols = mycolors) + NoLegend()
```
...
freqMatrix_df$category[!freqMatrix_df$fine_celltype %in% c("DC","Mac_IFN")] <- "Stable/Up"
...
ggplot(freqMatrix_df,aes(x = time,y=value*100,color=fine_celltype,group = group)) + geom_point() + geom_line() + 
  facet_wrap("~category") +
...
ifn.genes <- rownames(ifn.genes[ifn.genes$avg_log2FC > 1, & ifn.genes$p_val_adj < 0.05,])
```
...
Idents(combined.macro.metacells) <- "fine_celltype_2"
...
```{r}
Idents(combined.macro.metacells) <- "fine_celltype_2"
markers.rna <- FindAllMarkers(combined.macro.metacells,assay = "RNA",only.pos = T)
...
apply(differential.activity.strong)
# differential.activity.strong <- differential.activity.strong[differential.activity.strong$TF %in% markers.rna$gene,]

length(unique(differential.activity.strong$gene))

h.clust <- hclust(dist(x = combined.macro.metacells[["chromvar_pfm"]]@data[unique(differential.activity.strong$gene),]))

combined.macro.metacells[["chromvar_pfm"]]@data -> combined.macro.metacells[["chromvar_pfm"]]@scale.data
Idents(combined.macro.metacells) <- "fine_celltype_2"

DoHeatmap(combined.macro.metacells,assay = "chromvar_pfm",features = rownames(combined.macro.metacells[["chromvar_pfm"]]@data[unique(differential.activity.strong$gene),])[h.clust$order])



combined.macro.metacells[["chromvar_pfm"]]@scale.data <- combined.macro.metacells[["chromvar_pfm"]]@data
h.clust <- hclust(dist(x = combined.macro.metacells[["chromvar_pfm"]]@data))

Idents(combined.macro.metacells) <- "fine_celltype"
DoHeatmap(combined.macro.metacells,assay = "chromvar_pfm",features = rownames(combined.macro.metacells[["chromvar_pfm"]])[h.clust$order])
```
