...
DimPlot(combined.metacells,reduction = "wnn.umap",group.by = "celltype",cols = colors)
...
```{r}
# detectedMotifs <- rownames(combined.metacells@assays$chromvar) #motif
# relatedTFs <- ConvertMotifID(combined.metacells, id = detectedMotifs) #tf
```


```{r}
#VlnPlot(combined.metacells,features = ConvertMotifID(combined.metacells, name = c("Spi1","TCF7","FOXP3")),ncol = 2,group.by = "celltype")
```
...
Idents(combined.metacells) <- "wsnn_res.0.3"
...
```{r}
Idents(combined.metacells) <- "wsnn_res.0.03"
DefaultAssay(combined.metacells) <- "RNA"
...
```{r}
MoMacDcMarkers <- FindAllMarkers(combined.metacells[,combined.metacells$wsnn_res.0.3 %in% c(1,3,6,7)],only.pos = T)
MoMacDcMarkers <- MoMacDcMarkers[MoMacDcMarkers$p_val_adj < 0.05,]
head(MoMacDcMarkers[MoMacDcMarkers$cluster == 1,],n=15)
```
...
# markers2vs0 <- FindMarkers(combined.metacells,ident.1 = 2,ident.2 = 0) 
# markers2vs0 <- markers2vs0[markers2vs0$p_val_adj < 0.05,]
# markers2vs0[markers2vs0$avg_log2FC> 0.5,]
# markers2vs0[markers2vs0$avg_log2FC< 0.5,]


```
```{r}
VlnPlot(combined.metacells[,combined.metacells$wsnn_res.0.3 %in% c(0,3)], features = c("Nfkb1","Hdc","Il1b","Siglecf","Trem1","Cxcr2"),group.by = "wsnn_res.0.3")
```
```{r}
# Idents(combined.metacells) <- "wsnn_res.0.3"
# combined.metacells <- FindSubCluster(combined.metacells,cluster = c(0),resolution = 0.4,graph.name = "wsnn")
# 
# DimPlot(combined.metacells[,combined.metacells$wsnn_res.0.3 %in% c(0,2)],reduction = "wnn.umap",group.by = "sub.cluster")
# 
# FeaturePlot(combined.metacells[,combined.metacells$wsnn_res.0.3 %in% c(0,2)],reduction = "wnn.umap",features = "Siglecf")

```
```{r}
# Idents(combined.metacells) <- "sub.cluster"
# 
# markersNeutro <- FindAllMarkers(combined.metacells[,combined.metacells$wsnn_res.0.3 %in% c(0,2)],only.pos = T)
# markersNeutro <- markersNeutro[markersNeutro$p_val_adj < 0.05,]
# 
# markersNeutro[markersNeutro$cluster == "0_2",]
# markersNeutro[markersNeutro$cluster == "0_1",]
# markersNeutro[markersNeutro$cluster == "0_0",]
...
```{r}
FeaturePlot(combined.metacells[,combined.metacells$wsnn_res.0.3 %in% c(0,3)],
...
```{r}
# Idents(combined.metacells) <- "wsnn_res.0.3"
# combined.metacells <- FindSubCluster(combined.metacells,cluster = c(4),resolution = 0.4,graph.name = "wsnn")
# 
# DimPlot(combined.metacells,reduction = "wnn.umap",group.by = "sub.cluster")
# 
# Idents(combined.metacells) <- "sub.cluster"
# markers4_0 <- FindMarkers(combined.metacells,ident.1 = "4_0",ident.2 = "4_1")
# markers4_0 <- markers4_0[markers4_0$p_val_adj < 0.05,]
# markers4_0[markers4_0$avg_log2FC> 0,]
```
...
Idents(combined.metacells) <- 'wsnn_res.0.3'
combined.metacells <- FindSubCluster(combined.metacells,cluster = 4,resolution = 0.5,graph.name = "wsnn")
DimPlot(combined.metacells[,combined.metacells$wsnn_res.0.3 == 4],group.by = 'sub.cluster',reduction = "wnn.umap",label = T)

```
```{r}
DimPlot(combined.metacells[,combined.metacells$wsnn_res.0.3 == 4],group.by = 'sub.cluster',reduction = "wnn.umap",label = T)
VlnPlot(combined.metacells[,combined.metacells$wsnn_res.0.3 %in% c(4)],features = c("Cd8a","Cd4","Tox","Tcf7","Il7r","Ncr1","Klrb1c", "Klrb1b"),group.by="sub.cluster")
```
...
combined.metacells <- FindSubCluster(combined.metacells,cluster = 5,resolution = 0.4,graph.name = "wsnn")

DimPlot(combined.metacells[,combined.metacells$wsnn_res.0.3 == 5],reduction = "wnn.umap",group.by = "sub.cluster")

FeaturePlot(combined.metacells[,combined.metacells$wsnn_res.0.3 == 5],reduction = "wnn.umap",features  = c("Cd8a","Cd4","Tcrg-C1"))

VlnPlot(combined.metacells[,combined.metacells$wsnn_res.0.3 == 5], group.by = "sub.cluster",features  = c("Cd8a","Cd4","Foxp3","Tcrg-C1","Il2ra"))
...
levels(combined.metacells$fine_celltype) <- c(levels(combined.metacells$fine_celltype),c("T_reg",'gd_T'))
combined.metacells$fine_celltype[combined.metacells$fine_celltype == 5] <- Idents(combined.metacells[,combined.metacells$fine_celltype == 5])
...
combined.metacells <- FindSubCluster(combined.metacells,cluster = 2,resolution = 0.1,graph.name = "wsnn")

DimPlot(combined.metacells[,combined.metacells$wsnn_res.0.3 == 2],reduction = "wnn.umap",group.by = "sub.cluster")

FeaturePlot(combined.metacells[,combined.metacells$wsnn_res.0.3 == 2],reduction = "wnn.umap",features  = c("Clec10a","Ciita","Flt3","Fn1"))

VlnPlot(combined.metacells[,combined.metacells$wsnn_res.0.3 == 2], group.by = "sub.cluster",features  = c("Clec10a","Ciita","Flt3","Fn1"))
```
```{r}
Idents(combined.metacells) <- "sub.cluster"
combined.metacells <- RenameIdents(combined.metacells,
...
combined.metacells$fine_celltype[combined.metacells$fine_celltype == 2] <- Idents(combined.metacells[,combined.metacells$fine_celltype == 2])
...
combined.metacells <- FindSubCluster(combined.metacells,cluster = 0,resolution = 0.1,graph.name = "wsnn")

DimPlot(combined.metacells[,combined.metacells$wsnn_res.0.3 == 0],reduction = "wnn.umap",group.by = "sub.cluster")


VlnPlot(combined.metacells[,combined.metacells$wsnn_res.0.3 == 0], group.by = "sub.cluster",features  = c("Siglecf","Nfkb1","Trem1","Icam1"))
```
...
combined.metacells <- RenameIdents(combined.metacells,
                                   "0_0" = "Neutro_inf_low",
                                   "0_1" = "Neutro_Siglecf_high"
                                   )
levels(combined.metacells$fine_celltype) <- c(levels(combined.metacells$fine_celltype),c("Neutro_inf_high","Neutro_inf_low",'Neutro_Siglecf_high'))
combined.metacells$fine_celltype[combined.metacells$wsnn_res.0.3 %in% c(0)] <- Idents(combined.metacells[,combined.metacells$fine_celltype  %in% c(0)])
...
plot <- DimPlot(combined.metacells,reduction = "wnn.umap",group.by = "fine_celltype",cols = my36colors)
LabelClusters(plot, id = "fine_celltype", size = 5, repel = T,  box.padding = 1)
```

```{r}
saveRDS(combined.metacells,"immune.combined.metacells.annotated.local.frags.rds")
```
